	...
	methods: {
		...
		doGen: function () {
			...
			for (let items of this.res) {
				var content = items["item"][0]["lft"] +" "+items["item"][0].operator+" "+items["item"][0]["rgt"]+" "+'='+" "+items["item"][0]["result"]
				var dic = {
					questionText : items["item"][0]["lft"] +" "+items["item"][0].operator+" "+items["item"][0]["rgt"]+" "+'='+" ",
					answer: items["item"][0]["result"]
				}
				swiftarray.push(content)
			}
			window.webkit.messageHandlers.getHtmlArrayResponse.postMessage(swiftarray)
		},
